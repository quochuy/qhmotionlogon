...
$postData = '';
// Will contain post data from previous page.
...
if ( $Module->isCurrentAction( 'Login' ) and $Module->hasActionParameter( 'UserLogin' ) and ($Module->hasActionParameter( 'UserPassword' ) or $Module->hasActionParameter( 'UniStroke' )) and !$http->hasPostVariable( "RegisterButton" ) )
{
    ...
    // QH Motion Logon
    $UnistrokePoints = $Module->actionParameter( 'UniStroke' );
    ...
    if ( $userLogin != '' )
    {
        ...
        foreach ( array_keys ( $loginHandlers ) as $key )
        {
            ...
            // QH Motion Logon
            if ( $loginHandler == "qhmotionlogon" )
            {
                $user = $userClass->loginUser( $userLogin, $UnistrokePoints );
            }
            else
            {
            ...
            else
            {
                ...
                //    if($user == eZYubiKeyLoginUser::REQUIRE_MULTIFACTOR ||
                // $user == eZYubiKeyLoginUser::REQUIRE_YUBIKEY_OTP) break;
            }
        }
    ...
    if ( is_object( $user ) && !$haveRedirectionURI )
    {
         ...
         * 1. Per-user.
         * 2. Per-group.
         *    If the user object is published under several groups, main node is
         * chosen
         *    (it its URI non-empty; otherwise first non-empty URI is chosen from
         * the group list -- if any).
         *
         * See doc/features/3.8/advanced_redirection_after_user_login.txt for
         * more information.
         */

        // First, let's determine which attributes we should search redirection
        // URI in.
        $userUriAttrName = '';
        $groupUriAttrName = '';
        $ini = eZINI::instance( );
        ...
        if ( $userUriAttrName )
        {
            ...
            {
                eZDebug::writeWarning( "Cannot find redirection URI: there is no attribute '$userUriAttrName' in object '" . $userObject->attribute( 'name' ) . "' of class '" . $userObject->attribute( 'class_name' ) . "'." );
            }
        ...
        // 2.Check if redirection URI is specified for at least one of the user's
        // groups (preferring main parent group).
        if ( !$userUriSpecified && $groupUriAttrName && $user->hasAttribute( 'groups' ) )
        {
            ...
            if ( isset( $groups ) && is_array( $groups ) )
            {
                ...
                foreach ( $groups as $groupID )
                {
                    ...
                    {
                        eZDebug::writeWarning( "Cannot find redirection URI: there is no attribute '$groupUriAttrName' in object '" . $group->attribute( 'name' ) . "' of class '" . $group->attribute( 'class_name' ) . "'." );
                        continue;
                    }
                ...
                if ( $chosenGroupURI )// if we've chose an URI from one of the
                    // user's groups.
...
// Should we show message about failed login attempt and max number of failed
// login
