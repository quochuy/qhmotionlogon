...
<ul>
    ...
    {else}
     ...
     <li>{'QH Motion Logon'|i18n( 'design/admin/user/login' )}</li>
...
<form name="loginform" method="post" action={'/mlogon/login/'|ezurl}>
...
<div class="block">
    <label for="id3">{'Motion Logon (if set in your profile)'|i18n( 'design/admin/user/login' )}:</label>
    <div id="mlogon">
        <div id="mc">
            <canvas id="mc-canvas"></canvas>
        </div>
    </div>
    <input type="hidden" name="UniStroke" id="unistroke" />
{literal}
    <script type="text/javascript">
        $(document).ready(function(){
            $('#mlogon').motionCanvas({brushColor: [225, 225, 225], clearAfter: true});
        });
    </script>
{/literal}
</div>
