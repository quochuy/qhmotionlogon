 ...
 *
 * Copyright (c) Mateusz Krzeszowiak - matkrzesz[at]gmail[dot]com |
 * http://twitter.com/matkrzesz
 *
...
class phpDollar {
    ...
    /**
     * Class constructor, you can add here predefined templates by addTemplate()
     * function
     */
     ...
     * @return array Returns array with two keys: 'strokeName' - recognized
     * stroke and 'templScore' - strokes similarity score
     */
    public function recognizeStroke( $pointsArray )
    {
        ...
        $templatesScores = array( );
        for ( $i = 0; $i < $templatesCount; $i++ )
        {
            $distance = $this->_optimalCosineDistance( $this->_templatesArray[$i]['templVector'], $strokeVector );

            $templatesScores[] = array( 'strokeName' => $this->_templatesArray[$i]['templName'], 'strokeScore' => 1.0 / $distance );
            if ( $distance < $bestFit )
            {
                $bestFit = $distance;
                $t = $i;
        ...
        return array( 'strokeName' => $this->_templatesArray[$t]['templName'], 'strokeScore' => 1.0 / $bestFit, 'allScores' => $templatesScores );
    }
    ...
    public function addTemplate( $templName, $pointsArray )
    {
        ...
        $this->_templatesArray[] = array( 'templName' => $templName, 'templPoints' => $pointsArray, 'templVector' => $strokeVector );
    }
    ...
    private function _resampleTemplate( &$pointsArray, $numPoints )
    {
        ...
        for ( $i = 1; $i < count( $pointsArray ); $i++ )
        {
            ...
            if ( ($D + $pointsDistance) >= $intervalLenght )
            {
                ...
                array_splice( $pointsArray, $i, 0, array( $newPoint ) );
                // I hate this function... really. 3h just to wrap $newPoint with
                // array()...
                $D = 0.0;
            }
            else
    ...
    private function _vectorizeStroke( $pointsArray )
    {
        // „for Protractor"
        $pointsSum = 0.0;
        $vectorArray = array( );
    ...
    private function _optimalCosineDistance( $vectorOne, $vectorTwo )
    {
        // „for Protractor"
        $a = $b = 0.0;
        // I've got no idea what are these variables representing :P
